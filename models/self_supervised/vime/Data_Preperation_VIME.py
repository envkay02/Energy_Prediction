...
# To switch datasets, replace the end part of the file name accordingly.
...
train_labels = train_labeled['Label'] - 2
test_features = test_labeled.drop(columns='Label')
test_labels = test_labeled['Label'] - 2
...
num_classes = 9
y_train_one_hot = np.eye(num_classes)[resampled_labels.astype(int)]
y_test_one_hot = np.eye(num_classes)[test_labels.astype(int)]
